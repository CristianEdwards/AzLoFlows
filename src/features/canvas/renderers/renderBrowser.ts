...
/**
 * Renders a proper isometric 3D tablet/device with browser chrome content.
 * Solid colored body + glass screen on top face with title bar,
 * address bar, and content blocks.
 */
export function renderBrowser(
  ...
  // ── Browser content on glass screen ──
  ...
  if (showDetail) {
    ...
    const abBot = abTop + 0.07;
    drawPolygon(ctx, [tp(si + 0.06, abTop), tp(1 - si - 0.06, abTop), tp(1 - si - 0.06, abBot), tp(si + 0.06, abBot)]);
    ...
    for (let row = 0; row < 2; row++) {
      for (let col = 0; col < 2; col++) {
        const bTop = ct + row * 0.22;
        const bLeft = si + 0.04 + col * 0.44;
        const bW = 0.38;
        const bH = 0.16;
        drawPolygon(ctx, [tp(bLeft, bTop), tp(bLeft + bW, bTop), tp(bLeft + bW, bTop + bH), tp(bLeft, bTop + bH)]);
